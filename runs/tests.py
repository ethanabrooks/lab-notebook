...
def check_db(path, args):
    with DB as db:
        # check known values
        runs = db.get(path + "%")
        assert_in(DESCRIPTION, lookup.string(runs=runs, key="description"))
        assert_in(COMMAND, lookup.string(runs=runs, key="command"))
        assert_in(path, lookup.string(runs=runs, key="path"))
        for arg in args:
            assert_in(arg, lookup.string(runs=runs, key="command"))
...
def test_lookup():
    with _setup(TEST_RUN), DB as db:
        for key, value in dict(
            path=TEST_RUN, description=DESCRIPTION, command=COMMAND
        ).items():
...
def test_chdesc():
    with _setup(TEST_RUN), DB as db:
        ...
        assert_in(
            description, lookup.string(runs=db.get([TEST_RUN]), key="description")
        )
...
def test_move_dirs():
    ...
    with _setup("test_run1", args=["--run1"]), _setup("test_run2", args=["run2"]):
        ...
        with DB as db:
            assert_in(
                "--run1", lookup.string(runs=db.get(["test_run2"]), key="command")
            )
